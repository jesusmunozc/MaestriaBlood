...
export default function RateExperience() {
  ...
  return (
    <div className="min-h-screen bg-app-bg flex flex-col pb-28 page-enter">
      <PageHeader title="Calificar experiencia" onBack={() => navigate(-1)} />

      <div className="flex-1 flex flex-col items-center px-6 pt-6">
        {/* Rated user */}
        <div className="w-20 h-20 rounded-2xl bg-app-card border border-app-border/10 flex items-center justify-center text-3xl font-extrabold text-white mb-3">
          {ratedUser?.full_name?.charAt(0) ?? "?"}
        </div>
        <p className="text-white font-bold text-lg">
          {ratedUser?.full_name ?? "Usuario"}
        </p>
        <p className="text-app-text/40 text-sm mb-6">
          @{ratedUser?.username ?? "..."}
        </p>

        {/* Impartiality notice */}
        <div className="w-full bg-app-card border border-app-border/8 rounded-2xl p-4 mb-6 flex items-start gap-3">
        ...
        {/* Stars */}
        <p className="text-app-text/40 text-sm mb-3">¿Cómo fue tu experiencia?</p>
        ...
        <div className="w-full mt-6">
          <textarea
            ...
            rows={4}
            className="w-full bg-app-card border border-app-border/10 rounded-2xl px-4 py-3 text-white text-sm placeholder-white/25 resize-none focus:outline-none focus:border-blood-600/50 transition-colors"
          />
        </div>
      </div>
