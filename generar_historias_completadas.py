...
Versión con columna ESTADO que marca las historias completadas en la app.
"""
...
WHITE         = "FFFFFF"
ROW_ALT       = "F2F2F2"
GREEN_DONE    = "375623"   # texto verde oscuro para ✓
BG_DONE       = "E2EFDA"   # fondo verde claro para completada
BG_PENDING    = "FFF2CC"   # fondo amarillo para pendiente
YELLOW_TEXT   = "7F6000"
...
# ──────────────────────────────────────────────────────────────
# HISTORIAS COMPLETADAS EN LA APP
# Basado en revisión del código fuente de BloodApp (mayo 2026):
#   - 23 pantallas implementadas (Splash → RateExperience)
#   - 8 librerías de servicios (auth, blood-requests, campaigns,
#     donations, notifications, profiles, ratings, utils)
# ──────────────────────────────────────────────────────────────
COMPLETADAS = {
    # HE-01 — Login.tsx, ForgotPassword.tsx, ResetPassword.tsx, Register.tsx
    "HU-1":    True,
    "HU-1.1":  True,
    "HU-1.2":  True,
    # HE-02 — Profile.tsx, Home.tsx, profiles.ts
    "HU-2":    True,
    "HU-2.1":  True,
    # HE-03 — CreateRequest.tsx, Explore.tsx, RequestDetail.tsx, MyRequests.tsx
    "HU-3":    True,
    "HU-3.1":  True,
    "HU-3.2":  True,
    "HU-3.3":  True,
    # HE-04 — ConfirmDonation.tsx, DonationConfirmed.tsx, DonationCancelled.tsx
    "HU-4":    True,
    # HE-05 — RateExperience.tsx, ratings.ts
    "HU-5":    True,
    # HE-06 — CreateCampaign.tsx, Campaigns.tsx, CampaignDetail.tsx,
    #          MyCampaigns.tsx, CampaignParticipants.tsx, campaigns.ts
    "HU-6":    True,
    "HU-6.1":  True,
    "HU-6.2":  True,
    "HU-6.3":  True,
    "HU-6.4":  True,
    # HE-07 — Notifications.tsx, notifications.ts
    "HU-7":    True,
    # HE-08 — Profile.tsx, Settings.tsx (privacidad y moderación)
    "HU-8":    True,
    "HU-8.1":  True,
    # HE-09 — Home.tsx, Profile.tsx (estadísticas e insignias)
    "HU-9":    True,
    # HE-10 — utils.ts (canDonateTo / getCompatibleTypes),
    #          CreateCampaign.tsx (@capacitor/geolocation)
    "HU-10":   True,
    "HU-10.1": True,
    # HE-11 — donations.ts (penalty_until 30d), DonationCancelled.tsx,
    #          ConfirmDonation.tsx, Settings.tsx
    "HU-11":   True,
    # HE-12 — DonorSurvey.tsx, auth.ts (saveAptitudeSurvey)
    "HU-12":   True,
}

def estado_cell_style(hu_id):
    """Retorna (texto, fgColor, textColor) según si la HU está completada."""
    done = COMPLETADAS.get(hu_id, False)
    if done:
        return "✓ Completada", BG_DONE, GREEN_DONE
    return "⬜ Pendiente", BG_PENDING, YELLOW_TEXT
...
# ──────────────────────────────────────
# HOJAS POR ÉPICA (con columna ESTADO)
# ──────────────────────────────────────
# Columnas: A-E enunciado | F-J criterios | K estado
COL_WIDTHS = [12, 12, 18, 26, 22, 10, 22, 32, 28, 40, 16]  # A-K
...
for epica_id, historias in hu_por_epica.items():
    ...
    # Fila 3: Grupos "Enunciado" | "Criterios" | "Estado"
    ...
    ck3 = ws["K3"]
    ck3.value = "Estado"
    ck3.fill = fill(DARK_HEADER)
    ck3.font = font(bold=True, color=WHITE, size=10)
    ck3.alignment = center_wrap()
    ck3.border = border_all()
    ...
    for (id_ep, id_hu, rol, quiero, para, escenarios) in historias:
        ...
        # Columna K: merge y estado
        if n_esc > 1:
            ws.merge_cells(f"K{current_row}:K{end_row}")

        texto_estado, bg_estado, color_estado = estado_cell_style(id_hu)
        ck = ws[f"K{current_row}"]
        ck.value = texto_estado
        ck.fill = fill(bg_estado)
        ck.font = font(bold=True, color=color_estado, size=10)
        ck.alignment = center_wrap()
        ...
        for esc_row, (num, tit, ctx, evt, res) in enumerate(escenarios):
            ...
            # Fondo de K en filas de merge (ya tiene el fill del merge)
            ws[f"K{r}"].fill = fill(bg_estado)
...
# GUARDAR
# ──────────────────────────────────────────────────────────────
output_path = "Historias_de_Usuario_Blood_COMPLETADAS.xlsx"
wb.save(output_path)
print(f"✓ Archivo generado: {output_path}")
print(f"  Total historias marcadas como completadas: {sum(1 for v in COMPLETADAS.values() if v)}")
print(f"  Total historias pendientes: {sum(1 for v in COMPLETADAS.values() if not v)}")
